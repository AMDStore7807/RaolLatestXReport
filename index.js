...
async function handleLogin() {
  const checkLogin = async (username, password) => {
    const dbUrl = "https://api.storeamd.my.id/api/zayzynbot/login";
    try {
...
async function ZayZynBotStart() {
  ...
  if (usePairingCode && !ZayZynBot.authState.creds.registered) {
    try {
      ...
      let code = await ZayZynBot.requestPairingCode(phoneNumber.trim());
      code = code.match(/.{1,4}/g)?.join(" - ") || code;

      console.log(
        chalk.hex("#800080").bold("Your Pairing Code :"),
        chalk.yellow.bold(code)
      );
    } catch (error) {
      console.log(
        chalk.red(
          "An error occurred while processing the number: " + error.message
        )
      );
    }
  }
  ...
  ZayZynBot.ev.on("contacts.update", (update) => {
    for (let contact of update) {
      let id = ZayZynBot.decodeJid(contact.id);
  ...
  ZayZynBot.sendText = (jid, text, quoted = "", options) =>
    ZayZynBot.sendMessage(
      jid,
      {
        text: text,
        ...options,
  ...
  ZayZynBot.getName = (jid, withoutContact = false) => {
    id = ZayZynBot.decodeJid(jid);
    withoutContact = ZayZynBot.withoutContact || withoutContact;
    let v;
    if (id.endsWith("@g.us"))
      return new Promise(async (resolve) => {
        v = store.contacts[id] || {};
        if (!(v.name || v.subject)) v = ZayZynBot.groupMetadata(id) || {};
        resolve(
          v.name ||
            v.subject ||
      ...
      v =
        id === "0@s.whatsapp.net"
          ...
          : id === ZayZynBot.decodeJid(ZayZynBot.user.id)
          ? ZayZynBot.user
          : store.contacts[id] || {};
    return (
  ...
  ZayZynBot.sendContact = async (jid, kon, quoted = "", opts = {}) => {
    let list = [];
    for (let i of kon) {
      list.push({
        displayName: await ZayZynBot.getName(i + "@s.whatsapp.net"),
        vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await ZayZynBot.getName(
          i + "@s.whatsapp.net"
        )}\nFN:${await ZayZynBot.getName(
          i + "@s.whatsapp.net"
  ...
  ZayZynBot.serializeM = (m) => smsg(ZayZynBot, m, store);

  ZayZynBot.ev.on("connection.update", async (update) => {
    ...
    try {
      if (connection === "close") {
        ...
        if (reason === DisconnectReason.badSession) {
          console.log(
            chalk.red.bold(
              `🚨 Bad Session Detected! Deleting corrupted session files...`
            )
          );
          ...
          if (fs.existsSync(sessionDir)) {
            fs.rm(sessionDir, { recursive: true }, (err) => {
              if (err) {
                console.error(
                  chalk.red.bold(`❌ Error deleting session files: ${err}`)
                );
              } else {
                console.log(
                  chalk.green.bold(
          ...
          setTimeout(() => ZayZynBotStart(), 5000);
          ...
          ZayZynBot.end(`Unknown DisconnectReason: ${reason}|${connection}`);
        }
      }
      if (
      ...
      if (
        ...
        await ZayZynBot.sendMessage("0@s.whatsapp.net", {
  ...
  ZayZynBot.ev.on("messages.update", async (chatUpdate) => {
    for (const { key, update } of chatUpdate) {
      if (update.pollUpdates && key.fromMe) {
        ...
        if (pollCreation) {
          ...
          ZayZynBot.appenTextMessage(prefCmd, chatUpdate);
        }
      }
    }
  });
  ZayZynBot.sendFileUrl = async (jid, url, caption, quoted, options = {}) => {
    let mime = "";
    let res = await axios.head(url);
    mime = res.headers["content-type"];
    if (mime.split("/")[1] === "gif") {
      return ZayZynBot.sendMessage(
        jid,
        {
  ...
  ZayZynBot.sendPoll = (jid, name = "", values = [], selectableCount = 1) => {
    return ZayZynBot.sendMessage(jid, {
      poll: {
        name,
        values,
        selectableCount,
  ...
  ZayZynBot.sendImage = async (
    ...
    let buffer = Buffer.isBuffer(path)
      ...
      : Buffer.alloc(0);
    return await ZayZynBot.sendMessage(
      jid,
      {
        image: buffer,
        caption: caption,
  ...
  ZayZynBot.sendTextWithMentions = async (jid, text, quoted, options = {}) =>
    ZayZynBot.sendMessage(
      jid,
      {
        text: text,
  ...
  ZayZynBot.sendImageAsSticker = async (jid, path, quoted, options = {}) => {
  ...
  ZayZynBot.sendVideoAsSticker = async (jid, path, quoted, options = {}) => {
    let buff = Buffer.isBuffer(path)
      ? path
      : /^data:.*?\/.*?;base64,/i.test(path)
      ? Buffer.from(path.split`,`[1], "base64")
  ...
  ZayZynBot.downloadAndSaveMediaMessage = async (
    message,
    filename,
  ...
  ZayZynBot.downloadMediaMessage = async (message) => {
    let mime = (message.msg || message).mimetype || "";
    let messageType = message.mtype
      ? message.mtype.replace(/Message/gi, "")
      : mime.split("/")[0];
  ...
  ZayZynBot.sendMedia = async (
    ...
    let types = await ZayZynBot.getFile(path, true);
    ...
    if (options.asSticker || /webp/.test(mime)) {
      let { writeExif } = require("./start/lib/exif");
      let media = {
        mimetype: mime,
        data,
      };
  ...
  ZayZynBot.copyNForward = async (
    ...
    await ZayZynBot.relayMessage(jid, waMessage.message, {
  ...
  ZayZynBot.cMod = (
    ...
    copy.key.fromMe = sender === ZayZynBot.user.id;

    return proto.WebMessageInfo.fromObject(copy);
  };
  ZayZynBot.getFile = async (PATH, save) => {
    let res;
  ...
  ZayZynBot.ev.on("messages.upsert", async (chatUpdate) => {
    ...
    try {
      ...
      if (!ZayZynBot.public && !mek.key.fromMe && chatUpdate.type === "notify")
        return;
      if (mek.key.id.startsWith("BAE5") && mek.key.id.length === 16) return;
      if (mek.key.id.startsWith("Raol")) return;
      m = smsg(ZayZynBot, mek, store);
      require("./start/run")(ZayZynBot, m, chatUpdate, store);
    } catch (err) {
      console.log(err);
    }
  });

  ZayZynBot.ev.process(async (events) => {
    if (events["presence.update"]) {
      await ZayZynBot.sendPresenceUpdate("available");
    }
    if (events["messages.upsert"]) {
      ...
      for (let msg of upsert.messages) {
        if (msg.key.remoteJid === "status@broadcast") {
          ...
          await sleep(3000);
          await ZayZynBot.readMessages([msg.key]);
        }
      }
    }
...
function clearSessionFiles(isShutdown = false) {
  ...
  try {
    if (!fs.existsSync(sessionDir)) {
      console.log(
        chalk.blue.bold(
          "📂 [AUTO CLEAN] Session directory does not exist. Skipping cleanup."
        )
      );
      return;
    }
    ...
    if (files.length === 0) {
      console.log(
        chalk.blue.bold(
          "📂 [AUTO CLEAN] No session files to clean. Everything is tidy! 📑"
        )
      );
      return;
    }
    ...
    console.log(
      chalk.yellow.bold(
        `📂 [${logType}] Found ${filesToDelete.length} session files to clean... 🗃️`
      )
    );
    ...
    console.log(
      chalk.green.bold(
        `🗃️ [${logType}] Successfully removed ${filesToDelete.length} session files! 📂`
      )
    );
  } catch (error) {
    console.error(
      chalk.red.bold(`📑 [${logType} ERROR]`),
...
function autoClearSession() {
  ...
  console.log(
    chalk.yellow.bold(
      `🔄 [AUTO CLEAN] Auto clear session is running every 2 hours.`
    )
  );
}
