...
const _cmdUser = JSON.parse(
  fs.readFileSync("./start/lib/database/commandUser.json")
);
const {
  addCountCmd,
  getPosiCmdUser,
...
const pendaftar = JSON.parse(
  fs.readFileSync("./start/lib/database/userNumber.json")
);
const userPrem = JSON.parse(
  fs.readFileSync("./start/lib/database/userPremium.json")
);

//================= { BASE } =================\\
module.exports = ZayZynBot = async (ZayZynBot, m, chatUpdate, store) => {
  try {
    var body =
    ...
    //================= { USER } =================\\
    var isAuthor = global.ownNumb
      .replace(/[^0-9]/g, "")
      .includes(m.sender.split("@")[0]);
    const botNumber = await ZayZynBot.decodeJid(ZayZynBot.user.id);
    ...
    const groupMetadata = m.isGroup
      ? await ZayZynBot.groupMetadata(m.chat).catch(() => null)
      : null;
    ...
    //================= { PREMIUM } =================\\
    const isPremium =
      isCreator || isCreator || checkPremiumUser(m.sender, userPrem);
    ...
    try {
      ppuser = await ZayZynBot.profilePictureUrl(m.sender, "image");
    } catch (err) {
      ppuser =
        "https://raw.githubusercontent.com/latesturl/dbCDN/refs/heads/main/my-DB/profile.jpg";
    }
    ...
    try {
      ...
      if (!("nick" in user)) user.nick = ZayZynBot.getName(m.sender);
    ...
    const reply = async (teks) => {
      ...
      return ZayZynBot.sendMessage(m.chat, ZayZynBotJob, {
        quoted: ftroli,
    ...
    const plugins = await pluginsLoader(
      path.resolve(__dirname, "../script/cjs")
    );
    ...
    //================= { CONSOLE LOGS } =================\\
    if (m.message) {
      let header = chalk.black(
        chalk.bgHex("#ff5e78").bold(`\n🌟 ${ucapanWaktu} 🌟`)
      );
      let title = chalk.white(
        chalk.bgHex("#4a69bd").bold("🚀 There is a message 🚀")
      );
      let date = chalk.cyanBright(
        `📅 DATE        : ${new Date().toLocaleString()}`
      );
      let sender = chalk.yellowBright(`🗣️  SENDERNAME  : ${pushname}`);
      let jid = chalk.magentaBright(`👤 JIDS        : ${m.sender}`);
      let mess = chalk.magentaBright(`👤 COMMAND     : ${command}`);
      if (isCmd && !m.isGroup) {
        console.log(header);
        console.log(title);
        console.log(date);
    ...
    if (isCmd && !isUser) {
      ...
      fs.writeFileSync(
        "./start/lib/database/userNumber.json",
        JSON.stringify(pendaftar, null, 2)
      );
    }

    //================= { SWITCH CASE } =================\\
    const caseHandler = require("../script/case/case");
    await caseHandler(
      ZayZynBot,
      m,
      command,
